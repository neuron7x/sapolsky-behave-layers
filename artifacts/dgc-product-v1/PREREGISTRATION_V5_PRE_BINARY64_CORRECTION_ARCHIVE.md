# DGC Product v1 — Archived Pre-execution Registration V5 (pre-binary64 correction)

Status: `SUPERSEDED_PRE_EXECUTION_PROTOCOL_V5 / EXTERNAL_CONFIRMATORY_OUTCOMES_UNOBSERVED`
...
Archived on 2026-08-24 before any external confirmatory outcome bytes were observed. Superseded because the decimal hardcode for `zeta(1.4)` was one binary64 ULP above the value produced by the pinned author-reference Boost path. The old value was slightly more conservative, but it was not an exact binary64 replay of the reference implementation. V5.1 corrects and content-identifies the numeric boundary parameter set.
