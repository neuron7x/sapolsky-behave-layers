...
def run_fault_injections() -> dict[str, bool]:
    ...
    decision = ComputeGovernor.select(
        operations=[op], estimates={op.operation_id: est}, budget=BudgetLedger(10, 10, 10), decision_digest="d"
    )
    ...
    try:
        PerturbationTemplate(
            "x", ("1",), InterventionType.CAUSAL_INTERVENTION, "attack", 1.0
        )
