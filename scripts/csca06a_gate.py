...
def main() -> int:
    ...
    if verdict.get('verdict') != 'INTERVENTIONAL_FALSIFIABILITY_INSTRUMENT_QUALIFIED_NARROWED': errors.append('unexpected verdict')
    if verdict.get('scientific_pass') is not True: errors.append('scientific_pass false')
    ...
    for cohort in ('primary_summary','replication_summary'):
        ...
        for k in ('S1_MISSING_TRUE_EDGE','S2_MISSING_TRUE_EDGE_NEGATIVE','S3_SPURIOUS_CANDIDATE_EDGE'):
            if c[k]['rejection_rate'] < 0.95: errors.append(f'{cohort}/{k} insufficient power')
    ...
    if errors:
        ...
        for e in errors: print(' -', e)
        return 1
    print('CSCA06A-GATE PASS: composite model-class falsification qualified narrowly; graph truth remains unauthorized.')
    return 0
