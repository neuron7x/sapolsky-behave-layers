...
from cwc.governance.frozen_mechanism_executor import execute_frozen_mechanism_panel
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Execute a frozen DGC cost+quality mechanism panel with no product-risk authority."
    )
    parser.add_argument("--execution-freeze", required=True)
    parser.add_argument("--harness-freeze", required=True)
    parser.add_argument("--mechanism-authority", required=True)
    parser.add_argument("--materialization-generation-root", required=True)
    parser.add_argument("--output-root", required=True)
    parser.add_argument("--worker-id", default="dgc-mechanism-executor")
    args = parser.parse_args()

    output = execute_frozen_mechanism_panel(
        repository_root=ROOT,
        execution_manifest_freeze_path=_path(args.execution_freeze),
        harness_freeze_path=_path(args.harness_freeze),
        mechanism_authority_path=_path(args.mechanism_authority),
    ...
    manifest = json.loads(
        (output / "MECHANISM_EXECUTION_BUNDLE.json").read_text(encoding="utf-8")
    )
    print(json.dumps({
        "status": "PASS",
