...
**ACT-R&D-03 remains negative for broad learned-model causal authority.** CSCA-06A-R1 closes a controlled model-class falsifiability subproblem only. The next admissible real-model gate is `CSCA-06B`: define an admissible intervention-operator equivalence class and test operator invariance before any amortized/student or semantic promotion.
