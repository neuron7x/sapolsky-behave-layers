...
**ACT-R&D-03 remains negative for broad learned-model causal authority.** CSCA-06A-R1 closes a controlled model-class falsifiability subproblem; CSCA-06B qualifies only operator-family robustness; CSCA-06C blocks a content-specific interpretation. Countermodel generation now blocks consolidation under surviving observational equivalence, but semantic causality, real-trace identification, student amortization, replay and active causal control remain blocked. Next hard gate: `COG-EPISTEMIC-01` typed authority lattice.
