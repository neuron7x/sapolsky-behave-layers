...
SCHEMA = "DGC_EXECUTED_P9_AUTHORITY_V1"
...
def build_executed_p9_authority(
    *,
    ...
    if execution_authority.get("execution_bundle_digest") != bundle.bundle_digest:
        raise ExecutedP9AuthorityError("P9 execution bundle differs from EXECUTED authority subject")
    if execution_authority.get("metric_population_digest") != sha256_bytes(canonical_json_bytes([
        ...
        raise ExecutedP9AuthorityError("P9 metric population differs from EXECUTED authority")
    ...
    payload = {
        ...
        "p9_supported": bool(certificate.pareto.all_baselines_certified),
    }
...
def verify_executed_p9_authority_document(path: Path) -> dict[str, object]:
    ...
    for name in (
        ...
        "execution_bundle_digest", "metric_population_digest", "harness_freeze_digest",
        "confirmatory_task_manifest_digest", "statistical_plan_digest",
        "paired_evidence_population_digest", "p9_certificate_digest",
    ):
