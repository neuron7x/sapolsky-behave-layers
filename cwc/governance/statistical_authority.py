...
class StatisticalInferenceCertificate:
    ...
    def digest(self) -> str:
        payload = {
            ...
            "sampling_trace_digest": self.sampling_trace_digest,
            "calibration_digest": self.calibration_digest,
            "drift_guard_digest": self.drift_guard_digest,
            "invalidated_by_drift": self.invalidated_by_drift,
            "method": self.method,
        }
    ...
    def admits(self, estimate: ValueOfComputationEstimate) -> bool:
        return (
            not self.invalidated_by_drift
            and self.operation_id == estimate.operation_id
            and self.estimate_digest == digest_voc_estimate(estimate)
            and all(x.strip() for x in (self.sampling_policy_digest, self.sampling_trace_digest, self.calibration_digest, self.drift_guard_digest))
        )
...
def certify_statistical_inference_authority(*, estimate: ValueOfComputationEstimate, scope: StatisticalScope, sampling_policy_digest: str, sampling_trace_digest: str, calibration_digest: str, drift_guard_digest: str, invalidated_by_drift: bool) -> StatisticalInferenceCertificate:
    ...
    return StatisticalInferenceCertificate(
        operation_id=estimate.operation_id,
        estimate_digest=digest_voc_estimate(estimate),
        scope=scope,
        sampling_policy_digest=sampling_policy_digest,
        sampling_trace_digest=sampling_trace_digest,
        calibration_digest=calibration_digest,
        drift_guard_digest=drift_guard_digest,
        invalidated_by_drift=bool(invalidated_by_drift),
    )
