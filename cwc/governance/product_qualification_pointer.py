...
def _sha(name: str, value: object) -> str:
    text = str(value)
    if text != text.strip() or text != text.lower():
        raise ProductQualificationPointerError(f"{name} must use canonical lowercase SHA-256 text")
...
def _oid(name: str, value: object) -> str:
    text = str(value)
    if text != text.strip() or text != text.lower():
        raise ProductQualificationPointerError(f"{name} must use canonical lowercase Git object-id text")
...
def _canonical_rel(value: object, *, label: str) -> str:
    ...
    if (
        ...
        raise ProductQualificationPointerError(f"{label} must be a canonical repository-relative POSIX path")
...
def _path_pair(name: str, value: object) -> tuple[str, str]:
    ...
    return (
        _canonical_rel(value[0], label=f"{name}[0]"),
        _canonical_rel(value[1], label=f"{name}[1]"),
    )


def _safe_repo_file(root: Path, value: object, *, label: str) -> Path:
    rel_text = _canonical_rel(value, label=label)
    candidate = root / PurePosixPath(rel_text)
...
def load_product_qualification_pointer(path: Path) -> dict[str, object]:
    ...
    _canonical_rel(doc.get("ledger_path"), label="ledger_path") if doc.get("activation_authorized") is True else None
    _canonical_rel(doc.get("global_v5_authority_path"), label="global_v5_authority_path") if doc.get("activation_authorized") is True else None
    _canonical_rel(doc.get("source_registry_path"), label="source_registry_path")
    _canonical_rel(doc.get("p19_verifier_policy_path"), label="p19_verifier_policy_path")
    for field in (
        "family_p19_paths", "family_attestation_paths", "family_verification_report_paths", "family_signature_paths"
    ):
        if doc.get("activation_authorized") is True:
            _path_pair(field, doc.get(field))
        else:
            value = doc.get(field)
            if not isinstance(value, list) or len(value) != 2:
                raise ProductQualificationPointerError(f"{field} must contain exactly two paths")
...
def verify_product_qualification_pointer(
    *,
    ...
    if not generation_id or generation_id != generation_id.strip() or generation_id == "UNCONFIGURED":
        raise ProductQualificationPointerError("product qualification generation_id is not canonically configured")
    repo_commit = _oid("repo_commit", doc.get("repo_commit"))
    repo_tree = _oid("repo_tree", doc.get("repo_tree"))
    if expected_repo_commit is not None and repo_commit != _oid("expected_repo_commit", expected_repo_commit):
        raise ProductQualificationPointerError("qualification pointer commit differs from expected execution commit")
    if expected_repo_tree is not None and repo_tree != _oid("expected_repo_tree", expected_repo_tree):
    ...
    row = evidence[0]
    receipt_rel = _canonical_rel(row.get("path", ""), label="PRODUCT_QUALIFIED receipt evidence path")
    pointer_global_path = global_path.relative_to(root).as_posix()
    if receipt_rel != pointer_global_path:
        raise ProductQualificationPointerError("PRODUCT_QUALIFIED receipt references a different global authority")
    if _sha("PRODUCT_QUALIFIED receipt sha256", row.get("sha256")) != global_sha:
        raise ProductQualificationPointerError("PRODUCT_QUALIFIED receipt/global authority SHA binding mismatch")
    try:
        receipt_bytes = int(row.get("bytes", -1))
    except (TypeError, ValueError) as exc:
        raise ProductQualificationPointerError("PRODUCT_QUALIFIED receipt byte count malformed") from exc
    if receipt_bytes != global_path.stat().st_size:
