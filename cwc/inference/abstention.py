...
def decide_causal_authority(
    ...
    *,
    ...
    if max_structural_evaluations is not None and structural_evaluations > max_structural_evaluations:
        return CreditAuthorityDecision("ABSTAIN_COMPUTE_BUDGET", None, "STRUCTURAL_EVALUATION_BUDGET_EXCEEDED", policy.version)
    ...
    if min(counts.values()) < policy.min_intervention_support:
        return CreditAuthorityDecision(
            "ABSTAIN_INSUFFICIENT_INTERVENTION_SUPPORT", None, "INTERVENTION_SUPPORT_BELOW_FROZEN_MINIMUM", policy.version
        )
    if max(envelope.observed_effect_magnitudes.values(), default=0.0) < policy.leverage_floor:
        return CreditAuthorityDecision("FALSIFIED_NO_LEVERAGE", None, "SUPPORTED_INTERVENTIONS_SHOW_NO_LEVERAGE", policy.version)
