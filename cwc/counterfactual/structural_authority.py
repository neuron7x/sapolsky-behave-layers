...
def decide_structural_authority(
    ...
    if envelope.covered_cells < policy.required_cells or envelope.min_cell_support < policy.min_cell_support:
        return StructuralAuthorityDecision(
            ...
            None,
            "INTERVENTION_COVERAGE_BELOW_FROZEN_MINIMUM",
            policy.version,
        )
    if envelope.max_empirical_leverage < policy.leverage_floor:
        return StructuralAuthorityDecision(
            ...
            None,
            "INTERVENTIONS_SHOW_NO_MEASURABLE_LEVERAGE",
            policy.version,
        )
    if envelope.max_cell_idr > policy.max_cell_idr:
        return StructuralAuthorityDecision(
            ...
            None,
            "INTERVENTIONAL_DIVERGENCE_EXCEEDS_FROZEN_CALIBRATION",
            policy.version,
        )
    if envelope.context_shift_candidates:
        return StructuralAuthorityDecision(
            "CONTEXT_CONDITIONAL_ONLY",
            envelope.best_model_family,
            "INTERVENTIONAL_EFFECT_CHANGES_ACROSS_CONTEXT",
            policy.version,
        )
    return StructuralAuthorityDecision(
        "STRUCTURAL_ADEQUACY_ACCEPTED_SYNTHETIC",
        envelope.best_model_family,
        "CALIBRATED_INTERVENTIONAL_ADEQUACY_PASSED",
        policy.version,
    )
