...
def test_idempotent_same_result_conflict_quarantines():
    ...
    first = coordinator.commit(
        lease, tick=1, result_payload={"x": 1}, evidence_digest="e" * 64, actual_cost_usd=0.2
    )
    assert coordinator.commit(
        lease, tick=2, result_payload={"x": 1}, evidence_digest="e" * 64, actual_cost_usd=0.2
    ) == first
    with pytest.raises(ValueError, match="conflicting duplicate"):
        coordinator.commit(
            lease, tick=2, result_payload={"x": 2}, evidence_digest="e" * 64, actual_cost_usd=0.2
        )
...
def test_stale_worker_cannot_commit_after_reassignment():
    ...
    with pytest.raises(ValueError, match="stale or forged lease"):
        coordinator.commit(
            old, tick=6, result_payload={"late": 1}, evidence_digest="e" * 64, actual_cost_usd=0.1
        )
    coordinator.commit(
        new, tick=6, result_payload={"ok": 1}, evidence_digest="e" * 64, actual_cost_usd=0.1
    )
...
def test_cost_cap_and_evidence_digest_hard():
    ...
    with pytest.raises(ValueError, match="per-unit cap"):
        coordinator.commit(
            lease, tick=1, result_payload={"x": 1}, evidence_digest="e" * 64, actual_cost_usd=1.01
        )
    with pytest.raises(ValueError, match="evidence_digest"):
        coordinator.commit(
            lease, tick=1, result_payload={"x": 1}, evidence_digest="short", actual_cost_usd=0.1
        )
...
def test_completion_refuses_partial_population():
    ...
    lease = coordinator.claim("w", tick=0)
    coordinator.commit(
        lease, tick=1, result_payload={"ok": 1}, evidence_digest="e" * 64, actual_cost_usd=0.1
    )
