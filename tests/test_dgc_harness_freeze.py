...
def execution_doc(path: Path, *, policy_ids=("B0", "B1", "B2", "B3", "DGC"), collide=False) -> Path:
    components = [
        ...
        for name, char in zip(
            ("action_catalog_manifest", "executor_manifest", "model_manifest", "observation_builder_manifest", "prompt_policy", "tool_manifest", "environment", "budget", "pricing_snapshot", "risk_endpoint_manifest", "scorer"),
            "123456789ab",
            strict=True,
        )
    ]
