...
def _pointer_doc(
    *,
    ...
    payload = {
        ...
        "global_v5_authority_path": global_path,
...
def _fixture(tmp_path: Path, monkeypatch, *, rebuilt_digest: str = "c" * 64):
    ...
    global_v5 = tmp_path / "artifacts/global-v5.json"
    _write_subject(global_v5, b"global-v5-bytes\n")
    for rel in (
    ...
    global_sha = sha256_file(global_v5)
    global_digest = "c" * 64
    _FakeLedger.state = _terminal_state("artifacts/global-v5.json", global_sha, global_v5.stat().st_size)
    ...
    _write_pointer(
        ...
        _pointer_doc(
            ...
            global_path="artifacts/global-v5.json",
            ledger_sha=sha256_file(ledger),
            global_sha=global_sha,
            global_digest=global_digest,
        ),
    )
    return ledger, global_v5, pointer


def test_pointer_v3_rebuilds_portable_global_v5_then_replays_terminal_ledger(tmp_path: Path, monkeypatch):
    ...
    assert verified.global_v5_authority_digest == "c" * 64
...
def test_self_consistent_declared_global_v5_cannot_substitute_for_failed_semantic_rebuild(tmp_path: Path, monkeypatch):
...
def test_execution_identity_mismatch_fails_before_evidence_acceptance(tmp_path: Path, monkeypatch):
    _, _, pointer = _fixture(tmp_path, monkeypatch)
...
def test_global_v5_byte_substitution_fails_closed(tmp_path: Path, monkeypatch):
    _, global_v5, pointer = _fixture(tmp_path, monkeypatch)
    global_v5.write_text("mutated\n", encoding="utf-8")
...
def test_terminal_receipt_cannot_reference_different_global_authority(tmp_path: Path, monkeypatch):
    _, global_v5, pointer = _fixture(tmp_path, monkeypatch)
    global_sha = sha256_file(global_v5)
    _FakeLedger.state = _terminal_state("artifacts/other-global.json", global_sha, global_v5.stat().st_size)
